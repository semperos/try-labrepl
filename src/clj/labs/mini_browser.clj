(ns ^{:next "Unified Update Model" :next-url "/labs/unified-update-model"
  ...
  (:use [compojure.core :only (defroutes GET)]
        ...
        [try-labrepl.views.layout :only (default-stylesheets default-javascripts)]
        [try-labrepl.util :only (c repl-showme showme showme* source)]
...
(defn overview []
  [[:h3 "Overview"]
   [:p "In this lab you will
       ...
       Compojure <strong><em>(TODO: Consider using Noir, since that's what try-labrepl is written in)</em></strong>. The browser will provide access to Clojure namespaces,
  ...
  [[:h3 "Layout"]
   ...
   [:ol
    ...
    [:li "The labs include a set of premade JavaScripts and CSS files. Use the namespace "
          (c try-labrepl.views.layout) ", and you can access these files via the vars "
    ...
    [:li "The " (c var-detail) " function should return markup:"
     [:ul
      ...
      [:li "The source code itself. You will need to use" (c find-var) ", "
            (c clojure.repl/source-fn) ", and " (c try-labrepl.util/code*) ", which will wrap
...
(defn bonus []
  [[:h3 "Bonus"]
   [:ol
    ...
    [:li "The lab you are reading is itself a Noir app, but shares many similarities with Compojure (most importantly, the underlying Ring library). Use your browser to explore
          the namespaces starting with " (c try-labrepl) "."]]])
